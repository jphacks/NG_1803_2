...
for _ in range(2):
    f = open(['format_data/suntoryCocktailRecipe_formatData_ja.json', 'translation_data/suntoryCocktailRecipe_translationData_en.json', 'translation_data/suntoryCocktailRecipe_translationData_zh.json'][_], 'r', encoding="utf-8_sig")
    ...
    for i in range(l):
        ...
        for j in range(1, 11):
            if not (exec("(ja_line['ingredient{}'])".format(j))):
                ingredients = [ingredients[0][:j - 1], ingredients[1][:j - 1]]
                amount_numbers = [amount_numbers[0][:j - 1], amount_numbers[1][:j - 1]]
                break
